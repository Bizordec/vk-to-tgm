...
async def main():
    ...
    tgm_search_client = TelegramClient("tgm_sessions/tgm_search_client", settings.TGM_API_ID, settings.TGM_API_HASH)
    ...
    tgm_bot = TelegramClient("tgm_sessions/tgm_bot", settings.TGM_API_ID, settings.TGM_API_HASH)
    await tgm_bot.start(bot_token=settings.TGM_BOT_TOKEN)
    await get_channel_entities(tgm_bot)
    await tgm_bot.disconnect()

    worker_bot = TelegramClient("tgm_sessions/worker_bot", settings.TGM_API_ID, settings.TGM_API_HASH)
    await worker_bot.start(bot_token=settings.TGM_BOT_TOKEN)
    await get_channel_entities(worker_bot)
    await worker_bot.disconnect()

    if settings.TGM_PL_CHANNEL_ID:
        worker_pl_bot = TelegramClient("tgm_sessions/worker_pl_bot", settings.TGM_API_ID, settings.TGM_API_HASH)
